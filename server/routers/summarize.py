...
async def summarize_text(req: SummarizeRequest, user=Depends(get_current_user)):
    ...
    has_speakers = bool(__import__("re").search(r"^発言者[A-Z]:", req.text, __import__("re").MULTILINE))
    system_prompt = (
        "あなたは議事録の要約を担当するアシスタントです。以下は話者ごとに分かれた会議の発言記録です。"
        "各話者の主要な発言・議論のポイントを整理し、会議全体を簡潔に要約してください。"
        if has_speakers
        else
        "あなたは議事録の要約を担当するアシスタントです。以下は会議の文字起こしテキストです。"
        "主要な議題・決定事項・次のアクションを箇条書きで簡潔に要約してください。"
    )
