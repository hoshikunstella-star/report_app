# File: stt/transcribe_segments.py
# Description: テキストの抽出処理（区間ごとに出力）
# Date: 2026-03-16
# Version: 1.0.0
...
def main() -> int:
    if len(sys.argv) < 2:  # 引数が2つ未満の場合はエラーメッセージを表示
        eprint("Usage: python transcribe_segments.py <audio_path>")
        return 2
    ...
    for seg in segments:
        ...
        if not text:  # テキストがない場合はスキップ
            continue
        out.append(  # 出力を追加
            {
                "start": float(seg.start) if seg.start is not None else None,  # 開始時間
                "end": float(seg.end) if seg.end is not None else None,  # 終了時間
                "text": text,  # テキスト
            }
        )

    sys.stdout.write(json.dumps(out, ensure_ascii=False))  # 出力をJSON形式で出力
    sys.stdout.flush()
    return 0
